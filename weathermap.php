...
$inhtml=file_get_html("Inverness(Highland).html");
$IN=getWeather($inhtml,0);
$abhtml=file_get_html("Aberdeen(Aberdeen).html");
$AB=getWeather($abhtml,0);
$edhtml=file_get_html("Edinburgh(Edinburgh).html");
$ED=getWeather($edhtml,0);
$behtml=file_get_html("Belfast(CountyAntrim).html");
$BE=getWeather($behtml,0);
$nehtml=file_get_html("NewcastleUponTyne(NewcastleuponTyne).html");
$NE=getWeather($nehtml,0);
$mahtml=file_get_html("Manchester(GreaterManchester).html");	// Get the latest weather, keep the html in the memory for later
$MA=getWeather($mahtml,0);
$sthtml=file_get_html("Stafford(Staffordshire).html");
$ST=getWeather($sthtml,0);
$cahtml=file_get_html("Cambridge(Cambridgeshire).html");
$CA=getWeather($cahtml,0);
$crhtml=file_get_html("GlamorganC.C.C.(Cardiff).html");
$CR=getWeather($crhtml,0);
$lohtml=file_get_html("London(GreaterLondon).html");
$LO=getWeather($lohtml,0);
$exhtml=file_get_html("Exeter(Devon).html");
