...
// Later updated/fixed by Max de Vos
...
function processPage($html,$when)
{
	...
	{
		$b= $html->find('b');
		if (!isset($b[$n]) || !isset($b[$t])) break;
		$time=	htmlspecialchars_decode ($b[$n]->plaintext,ENT_QUOTES);		// Decode html entities
		$time= str_replace(':', '', $time);
		$title=	html_entity_decode(htmlspecialchars_decode($b[$t]->plaintext, ENT_QUOTES), ENT_QUOTES, 'UTF-8');
