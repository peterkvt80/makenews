...
$container = $html->find('div#fiveDayText', 0);
$titles   = array();
$contents = array();
if ($container)
{
	$h4s = $container->find('h4');
	$ps  = array();
	foreach ($container->find('p') as $p)
	{
		if (strpos($p->getAttribute('class'), 'updated') === false)
			$ps[] = $p;
	}
	$pi = 0;
	foreach ($h4s as $h4)
	{
		if (isset($ps[$pi]))
		{
			...
			$contents[] = trim(preg_replace('/\s+/', ' ', $raw));
			$pi++;
		}
	}
}
...
foreach ($titles as $title)
{
	...
	$line   += outputline($line, 'F', $content, 22);
	$line++;
	if ($count==1)
	{
		$line=7;
